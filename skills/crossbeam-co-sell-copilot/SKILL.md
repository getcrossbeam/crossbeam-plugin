...
CRM or warehouse is preferred — it is the right tool for any pipeline filter and it gives the skill your own contacts on each deal, which sharpens the play. With no CRM, the connected ecosystem data resolves named accounts; for a pipeline filter, name or paste the deals.

**Optional but recommended**
- Gmail or Outlook connector — saves drafts directly to your inbox instead of copy-paste
- A reciprocity source (a CRM field, a sheet, or a note tracking what you owe each partner and what they owe you) — lets the draft reference the running give-and-get
...
Where accounts and deals come from — a CRM report or filter, a warehouse query, or a pasted list. CRM is preferred (pipeline filters and your-side contacts).
...
2. **An account/deal source (flexible).** CRM connector (preferred — exposes your contacts on the deal), warehouse connector, or a pasted list.
...
5. **Other GTM skills (optional).** Consult any installed skill that carries GTM context: an ICP, persona, or buying-center skill (committee definition for Step 2); a sales-playbook skill (frame the play and the ask); a voice skill for the sender (match the draft in Step 6); an account-brief or forecast/pipeline skill (richer context, and the natural source of an at-risk list to feed this skill). Consult when present; never hard-depend — the skill runs fully without any of them.
...
1. **CRM or warehouse (preferred, and the right tool for pipeline filters).** A filter like "stage 3" is a pipeline query — run it there. This is also where you get your contacts on each deal, which Step 2 needs.
2. **Crossbeam (fallback for named accounts).** With no CRM, resolve a named account by passing it to a `find_*` tool's `account_name` / `account_domain` (they fuzzy-match and return `ClarificationRequired` candidates to disambiguate). State the limit honestly: this resolves named accounts but will not enumerate your pipeline by stage — if the user gave a pipeline filter and there is no CRM/warehouse, ask them to name or paste the deals.
3. **Neither.** Ask the user to name or paste the accounts/deals. Do not guess.

For each, capture **account name**, **domain** when available, and — for an open deal from a CRM — **stage**, **amount**, and **the contacts already on the opportunity** (name, title, role). Your-side contacts make the coverage map possible; without them the play is coarser. Confirm any fuzzy set. If the set exceeds the volume threshold, stop and confirm.
...
- `missing_contacts` — the partner holds key contacts here that are not in your CRM (the multi-thread signal).
